...
def minutes_list(feed, stop_id):
    ...
    for trip in feed.filter_trips(headed_for_stop_id=stop_id):
        stop = next((s for s in trip.stop_time_updates if s.stop_id == stop_id), None)
        if not stop or not stop.arrival:
            continue
        mins.append(max(0, round((stop.arrival - now).total_seconds() / 60)))
...
def get_display_text():
    ...
    lines = [
        ...
        ", ".join(f"{m} mins" for m in manhattan),
        "-" * 40,
        "N (Coney Island-Bound):",
        ", ".join(f"{m} mins" for m in coney),
    ]
    return "\n".join(lines)
